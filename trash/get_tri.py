...
base = np.zeros( (tmstrandnum,3) )
base[0][0] = -1
base[-1][-1] = 1
tricenmtrx = base
for i in range(1,rep):
	tricenmtrx = np.vstack( ( tricenmtrx, base+i*np.ones( (tmstrandnum,3) ) ) )
print tricenmtrx
tricenmtrx = np.mod(tricenmtrx,rep) * sublen
print tricenmtrx

# then add the subtricen pattern to the basic tricenter matrix to get the final one
tricenmtrx = tricenmtrx + np.tile(subtricen, (rep,1))
#np.savetxt('inputs/{0}/{0}.tri.center'.format(pdb), tricenmtrx, fmt='%d', delimiter='\t')
print np.tile(subtricen, (rep,1))
print tricenmtrx
