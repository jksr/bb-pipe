...
for i in range(len(pdbs)):
	currpdb = pdbs[i]
	os.system('mkdir {name}/{currpdb}'.format(name=name,currpdb=currpdb))
	tmppdbs = pdbs[:i]+pdbs[i+1:]
	for type in types[:-1]:
		os.system('rm {name}/{currpdb}/{currpdb}.{type}'.format(name=name,type=type, currpdb=currpdb))
		for pdb in tmppdbs:
			os.system('cat meishan_data/{pdb}.{type} >> {name}/{currpdb}/{currpdb}.{type}'.format(name=name,pdb=pdb,currpdb=currpdb,type=type))


for type in types:
	os.system('rm {name}/all.{type}'.format(name=name,type=type))
	for pdb in pdbs:
		os.system('cat meishan_data/{pdb}.{type} >> {name}/all.{type}'.format(name=name,pdb=pdb,type=type))

import glob
for type in types[:-1]:
	for fn in glob.glob('{name}/*.{type}'.format(name=name, type=type))+glob.glob('{name}/*/*.{type}'.format(name=name, type=type)):
		os.system('perl calcodds.pl {fn}'.format(fn=fn))

for fn in glob.glob('{name}/*.{type}'.format(name=name, type='sidechain'))+glob.glob('{name}/*/*.{type}'.format(name=name, type='sidechain')):
	os.system('python singleodds_m.py {fn}'.format(fn=fn))
